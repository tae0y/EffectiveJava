...
- 필드변수로 클래스 타입을 구분하는 등 속임수를 쓰지말고, 계층구조를 제대로 활용하라

### 24. 멤버 클래스는 되도록 static으로 만들라
- 내부클래스 인스턴스가 바깥클래스 인스턴스와 독립적으로 존재할 수 있다면 정적 멤버클래스로 만들어야 한다
- 내부클래스가 바깥클래스 인스턴스의 멤버에 직접 접근해야 한다면 비정적 멤버클래스로 만들어야 한다
  - 그러나 내부클래스에서 바깥클래스 인스턴스로 숨은 레퍼런스를 계속 가지므로 가비지콜렉션을 막아 메모리 누수를 일으킬 수 있다
